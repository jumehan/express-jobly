...
describe("GET /users/:username", function () {
  ...
  test("works for useru1", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
  ...
  test("unauth user accessing another users page", async function () {
    const resp = await request(app)
      .get(`/users/u3`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("unauth user updating another user", async function () {
    const resp = await request(app)
      .patch(`/users/u2`)
      .send({
        firstName: "New",
...
describe("DELETE /users/:username", function () {
  ...
  test("unauth user deleting another user", async function () {
    const resp = await request(app)
      .delete(`/users/u2`)
      .set("authorization", `Bearer ${u1Token}`);
      expect(resp.statusCode).toEqual(401);
